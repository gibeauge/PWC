...
# put the output of the command "java -version" into an environment variable
  ...
  # now the environment variable contains "1.4.1"
  # remove the quotation marks
    JAVA_VERSION=`echo ${JAVA_VERSION} | sed 's/\"//g'`
  # now the environment variable containts 1.4.1 (without the quotations marks)
  ...
  # get the first digit of the java version: 1
	...
	echo "Cannot detect the Java-Runtime Version. Search embedded version ..."
    fi
  
    else
    echo "Cannot find the Java-Runtime: \"java\". Search embedded version ..."
    fi
...
then
    ...
    cd "$PRGDIR"
    exec "$JAVA_PATH" -jar "$PRGDIR/Uninstaller/uninstaller.jar" $* > /dev/null 2>&1 < /dev/null
